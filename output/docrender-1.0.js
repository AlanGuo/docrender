...
function(exports) {
    var tmpl = {
        apilisttemplate: /*<docs.render.apilisttemplate>*/'<% for(var item in data.classes){ %><li class="nav-header"><%=item%></li><%for(var j = 0, api; api = data.classitems[j]; j++){if(api["class"]==item && api.name && api.access!="private"){%><li><a href="#<%=item + "." + (api.name||"")%>"><%=item + "." + (api.name||"")%><% if(api.isNew){ %><span class="label label-success">New</span><% }else if(api.isModify){ %><span class="label label-info">Modify</span><% } %></a></li><%}}}%>'/*</docs.render.apilisttemplate>*/,
        apicontenttemplate: /*<docs.render.apicontenttemplate>*/'<% for(var item in data.classes){ %><h3 id="api.<%=item%>"><%=item.name%></h3><h4><%=data.classes[item].description || "" %></h4><table class="table table-striped table-hover table-bordered"><thead><tr><th>method&nbsp;<span class="badge"><%=data.classitems.filter(function(it){if(it["class"]==item && it.name && it.access!="private") return true;}).length %></span></th><th>details</th><th style="width: 60px;">v(iOS)</th><th style="width: 90px;">v(Android)</th></tr></thead><tbody><% for(var j = 0, api; api = data.classitems[j]; j++){if(api["class"]==item && api.name && api.access!="private"){%><tr><td id="<%=item + "." + (api.name||"")%>"><%=api.name %>&nbsp;<% if(api.isnew!=null){ %><span class="label label-success">New</span><% }else if(api.ismodify!=null){ %><span class="label label-info">Modify</span><% } %></td><td><%=api.description || "" %><br/><br/><% if(api.params){for(var k = 0, p; p = api.params[k]; k++){ %>@param<span class="param-type">{<%=encodeHTML(p.type) %>}</span><span class="param-param" title="<%=getSupportTips(p.support)%>"><%=p.name %></span><% if(p.support!=null){ %><sup class="notice"></sup><% } %><%=p.type == "Function" ? getArguList(p.props) : ""%><span class="param-desc"><%=p.description || ""%></span><br/><% if(p.props){ %><%=getParamsList(p.props) %><% } %><% }}%><% if(api["return"]){var p =api["return"];%>@return<span class="param-type">{<%=encodeHTML(p.type) %>}</span><span class="param-param" title="<%=getSupportTips(p.support)%>"><%=p.name %></span><% if(p.support!=null){ %><sup class="notice"></sup><% } %><%=p.type == "Function" ? getArguList(p.props) : ""%><span class="param-desc"><%=p.description || ""%></span><br/><% if(p.props){ %><%=getParamsList(p.props) %><% } %><% }%><% if(api.remark){ %><%=api.remark %><% } %><% if(api.note){ %><div class="alert"><b>Note</b> &nbsp;<%=api.note %></div><% } %><% if(api.important){ %><div class="alert alert-error"><b>Important</b> &nbsp;<%=api.important %></div><% } %><% if(api.example){ %><span class="label label-info">Example</span><pre class="text-info"><%=api.example%></pre><% } %><% if(api.changelist){ %><span class="label label-important">ChangeList</span><ul><% var verlist=api.changelist.match(/([^,]+?:[^,]+)/gi)||[];for(var i=0;i<verlist.length;i++){ %><li><%="v" + (/(.+):/i).exec(verlist[i])[1] + ": " + (/.+:(.+)/i).exec(verlist[i])[1]%></li><% } %></ul><% } %></td><td><% if(api.support){ %><%="v" + (/ios:([^,]+)/i).exec(api.support)[1] %><% }else{ %><span class="label label-warning">Unsupported</span><% } %></td><td><% if(api.support){ %><%="v" + (/android:([^,]+)/i).exec(api.support)[1] %><% }else{ %><span class="label label-warning">Unsupported</span><% } %></td></tr><% }} %></tbody></table><% } %>'/*</docs.render.apicontenttemplate>*/,
